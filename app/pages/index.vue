...
const title = computed(() => {
  ...
  return `Payslip (${startDay.padStart(
    2,
    "0"
  )} ${startMonth} - ${endDay} ${endMonth}, ${year})`;
});
  ...
  <div class="text-slate-600">
    <div :class="setting.fontFamily">
      <Transition name="help-slide">
        <div
          v-if="info.showInfo"
          class="no-print fixed top-0 left-0 z-50 w-full p-3"
        >
          <div
          ...
          >
            <div class="flex items-center justify-between px-5 py-3 bg-slate-50 border-b border-slate-100">
              ...
              <button
                class="cursor-pointer rounded-lg p-1.5 hover:bg-slate-200 transition-colors"
            ...
            <div class="px-5 py-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div class="flex items-start gap-3">
                ...
                <p class="text-sm text-slate-600">Click on any text to <strong class="text-slate-700">edit it directly</strong></p>
              </div>
              <div class="flex items-start gap-3">
                <UIcon name="i-lucide-palette" class="size-4 text-blue-500 mt-0.5 shrink-0" />
                <p class="text-sm text-slate-600">Use the <strong class="text-slate-700">color pickers</strong> or <strong class="text-slate-700">settings</strong> icon to change theme</p>
              </div>
              <div class="flex items-start gap-3">
                <UIcon name="i-lucide-type" class="size-4 text-blue-500 mt-0.5 shrink-0" />
                <p class="text-sm text-slate-600">Change <strong class="text-slate-700">font</strong> or <strong class="text-slate-700">currency</strong> from settings</p>
              </div>
              <div class="flex items-start gap-3">
                <UIcon name="i-lucide-printer" class="size-4 text-blue-500 mt-0.5 shrink-0" />
                <p class="text-sm text-slate-600">Click the <strong class="text-slate-700">print</strong> icon to print or download as PDF</p>
              </div>
              <div class="flex items-start gap-3 sm:col-span-2 bg-amber-50 rounded-lg px-3 py-2 border border-amber-100">
                <UIcon name="i-lucide-info" class="size-4 text-amber-500 mt-0.5 shrink-0" />
                <p class="text-sm text-amber-700">For best print results, set page margins to <strong>None</strong> in your browser's print settings</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div id="print-area" :style="cssVars">
        <div>
          <header
            class="flex justify-between items-start p-8 pb-6 border-b theme-border section mb-8"
          >
            <div>
              <Editable
                v-model="data.company.name"
                as="h1"
                class="text-3xl font-bold text-slate-700 mb-2"
              />
              <div class="leading-5">
                <Editable
            ...
            <div class="text-right relative text-slate-500">
              ...
              <Editable
                ...
                class="font-semibold absolute -bottom-6 right-0"
              />
            </div>
          </header>
          <main class="section">
            ...
            >
              <div class="w-40">
                ...
                <div class="text-lg font-medium text-slate-800 flex items-center gap-1">
                  ...
                  <input
                    type="date"
                    :value="toISODate(data.payslip.pay.date)"
                    class="no-print date-picker"
              ...
              <div class="w-72">
                ...
                <div
                  class="text-lg font-medium text-slate-800 flex items-center gap-1"
                >
                  <Editable
                  ...
                  <input
                    type="date"
                    :value="toISODate(data.payslip.period.start)"
                    class="no-print date-picker"
                  ...
                  <input
                    type="date"
                    :value="toISODate(data.payslip.period.end)"
                    class="no-print date-picker"
              ...
              <div
                id="netPay"
                class="bg-primary text-white rounded-lg px-6 pt-3 pb-3 transition-colors font-semibold"
              >
            ...
            <div class="grid grid-cols-2 gap-8 align-top mb-8">
              ...
              <div>
                ...
                />
                <div
                  v-for="(item, index) in data.bank.data"
                  :key="index"
                  class="flex"
                >
                  <Editable
                    :model-value="item.label"
                    class="font-semibold text-slate-600/90"
                  />:
                    ...
                    <tbody>
                      ...
                      >
                        <td class="tblCol">
                          <Editable
                            v-model="item.name"
                            class="font-medium whitespace-nowrap w-full"
                          />
                        </td>
                        <td class="tblCol">
                          <Editable
              ...
              <div class="col-span-5">
                ...
                <div class="no-print text-end mt-2">
                  <UButton
                    color="primary"
                    icon="heroicons-solid:plus"
                    label="Add Deduction Row"
                    class="cursor-pointer dark:text-slate-100"
                    @click="addDeductionRow"
                  />
                </div>
              </div>
            </div>
